...
The abstracted storage engine, as defined in the [FileStorage](./models/engine/file_storage.py) class, handles the management of classes such as the ones described. Each time the backend initializes, `HolbertonBnB` creates an instance of `FileStorage` named `storage`. This `storage` object loads or reloads data from class instances stored in the JSON file `file.json`. Whenever class instances are created, updated, or deleted, the `storage` object records these changes in the `file.json` file.

## Console :

The console serves as a command-line interpreter enabling the administration of the backend for HolbertonBnB. It facilitates the management and manipulation of all classes employed by the application, accomplished through interactions with the `storage` object as previously defined.
